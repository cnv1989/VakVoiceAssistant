...
def _get_chat_session_manager(session_id: str, actor_id: Optional[str] = None):
    ...
    if config.settings.agentcore_memory_id:
        try:
            ...
            memory_config = AgentCoreMemoryConfig(
                ...
                actor_id=actor_id or config.settings.agentcore_actor_id,
            )
...
async def whatsapp_chat(request: Request):
    """Chat endpoint for inbound WhatsApp messages via Twilio webhook.

    Similar to /twilio-chat but replies over WhatsApp and uses AgentCore
    sessions scoped per business number (session_id) and customer phone
    (actor_id).  This keeps conversation memory separate per business even
    though every customer texts the same WhatsApp number.
    """
    logger.info("whatsapp_chat endpoint called")

    body = await request.body()

    # Verify Twilio signature (same as /twilio-chat)
    if not verify_twilio_http_signature(request, body):
        is_production = config.settings.environment.value == "production"
        verification_enabled = config.settings.twilio_signature_verification_enabled
        if is_production and verification_enabled:
    ...
    # Parse the request body (Twilio sends form-encoded data)
    try:
        content_type = request.headers.get("Content-Type", "")
        if "application/json" in content_type:
            payload = json.loads(body.decode("utf-8"))
        ...
        logger.error(f"Failed to parse WhatsApp request body: {e}")
        raise HTTPException(status_code=400, detail="Invalid request body")
    ...
    # Twilio sends "To" = whatsapp:+14155238886 (our number),
    # "From" = whatsapp:+1XXXXXXXXXX (customer)
    raw_to = payload.get("To") or payload.get("to") or ""
    raw_from = payload.get("From") or payload.get("from") or ""
    whatsapp_number = raw_to.replace("whatsapp:", "") if raw_to else ""
    customer_phone = raw_from.replace("whatsapp:", "") if raw_from else ""

    if not customer_phone:
        raise HTTPException(status_code=400, detail="Customer phone (From) is required")

    # The WhatsApp number is shared across all businesses.
    # We need to figure out which business this customer was chatting with.
    # Use the business_number from the payload if available, otherwise
    # look up the customer's most recent booking link to find the business.
    business_number = (
        payload.get("business_number") or
        payload.get("businessNumber")
    )

    booking_link_record = None
    if not business_number and customer_phone:
        normalized_customer = normalize_phone_number(customer_phone) or customer_phone
        booking_link_record = await get_latest_booking_link(
            table_name=config.settings.user_booking_link_table,
            customer_phone=normalized_customer,
            aws_region=config.settings.aws_region,
        )
        if booking_link_record:
            business_number = booking_link_record.get("businessPhone", "")
            logger.info(
                "Resolved business from booking link: customer=%s -> business=%s",
                normalized_customer, business_number,
            )

    if not business_number:
        business_number = whatsapp_number

    logger.info("WhatsApp chat request: whatsapp_number=%s customer_phone=%s business_number=%s",
                whatsapp_number, customer_phone, business_number)
    ...
    try:
        ...
        if business_context.get("success"):
            logger.info(
                "Resolved context for WhatsApp chat: businessNumber=%s provider=%s services=%d",
                business_number,
                business_context.get("provider"),
                len(business_context.get("services") or []),
            )
        else:
            logger.warning("Failed to resolve business context for WhatsApp: %s", business_context.get("error"))
    except Exception as exc:
        logger.error("Failed to resolve business context for WhatsApp: %s", exc, exc_info=True)
        business_context = {"success": False, "error": str(exc)}
    ...
    system_prompt = base_prompt + config.WHATSAPP_PROMPT_SUFFIX if base_prompt else config.WHATSAPP_PROMPT_SUFFIX
    model_id = config.settings.bedrock_model_id
    max_tokens = config.settings.bedrock_max_tokens
    temperature = config.settings.bedrock_temperature

    # Session scoping:
    #   actor_id  = customer phone (the person texting)
    #   session_id = business number (keeps conversations per-business)
    normalized_customer = normalize_phone_number(customer_phone) or customer_phone
    normalized_business = normalize_phone_number(business_number) or business_number
    resolved_session_id = f"wa:{normalized_business}:{normalized_customer}"
    wa_actor_id = normalized_customer

    _chat_message_counts[resolved_session_id] = _chat_message_counts.get(resolved_session_id, 0) + 1
    emit_user_message_count("whatsapp_chat", resolved_session_id, _chat_message_counts[resolved_session_id])

    logger.info("WhatsApp agent session: session_id=%s actor_id=%s provider=%s model=%s",
                resolved_session_id, wa_actor_id, provider, model_id)
    ...
    try:
        ...
        business_context["current_local_time"] = get_localized_datetime_from_context(business_context)
        serializable_context = _make_json_serializable(business_context)
        session_manager = _get_chat_session_manager(resolved_session_id, actor_id=wa_actor_id)
        serializable_context = to_snake_case(serializable_context)
        initial_state = {"business_context": serializable_context, **serializable_context}
        agent = Agent(
            model=llm_model,
        ...
        for attempt in range(2):
            try:
                ...
                if is_tool_result_exceeds and attempt == 0:
                    logger.warning(
                        "WhatsApp session history toolResult mismatch; clearing and retrying: session_id=%s",
                        resolved_session_id,
                    )
                    try:
                        session_manager.delete_session(resolved_session_id)
        ...
        if is_max_tokens_error:
            logger.warning("WhatsApp agent hit max_tokens limit (max_tokens=%d).", max_tokens)
            emit_max_tokens_reached("whatsapp_chat", provider)
            emit_agent_metrics(
                endpoint="whatsapp_chat",
                provider=provider,
                duration_ms=(time.monotonic() - agent_start) * 1000,
            ...
            logger.error("WhatsApp agent invoke failed: %s", exc, exc_info=True)
            emit_agent_metrics(
                endpoint="whatsapp_chat",
                provider=provider,
                duration_ms=(time.monotonic() - agent_start) * 1000,
    ...
    try:
        ...
        twilio_auth_token = config.settings.twilio_auth_token
        if not twilio_account_sid or not twilio_auth_token:
            logger.error("Twilio credentials not configured for WhatsApp reply")
            raise HTTPException(status_code=500, detail="Twilio credentials not configured")

        twilio_client = TwilioClient(twilio_account_sid, twilio_auth_token)
        wa_from = raw_to if raw_to.startswith("whatsapp:") else f"whatsapp:{whatsapp_number}"
        wa_to = raw_from if raw_from.startswith("whatsapp:") else f"whatsapp:{customer_phone}"

        wa_message = twilio_client.messages.create(
            body=reply,
            from_=wa_from,
            to=wa_to,
        )
        ...
        emit_message_delivery("whatsapp", False)
        raise HTTPException(status_code=502, detail=f"Failed to send WhatsApp: {str(wa_exc)}")

    # Return empty TwiML (we already sent the message directly)
    return Response(
